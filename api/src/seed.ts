...
async function run() {
  ...
  if (!admin) {
    ...
    console.log('Created admin: admin@shop.dev / admin123');
  }
  ...
  const products = [
    {
      name: 'Áo thun cotton trắng', slug: 'ao-thun-cotton-trang', price: 250000, originalPrice: 320000,
      category: cats['ao'], badge: 'Mới',
    ...
    {
      name: 'Quần jean ống đứng', slug: 'quan-jean-ong-dung', price: 480000,
      category: cats['quan'], badge: 'Hot',
    ...
    {
      name: 'Đầm linen midi be', slug: 'dam-linen-midi-be', price: 690000,
      category: cats['dam'],
  ...
  for (const p of products) {
    let prod = await prodRepo.findOne({ where: { slug: p.slug } });
    if (prod) continue;
    ...
    await imgRepo.save(p.images.map((url, i) => imgRepo.create({ product: prod, url, position: i })));
    await varRepo.save(p.variants.map((v) => varRepo.create({ ...v, product: prod, priceDelta: '0' })));
